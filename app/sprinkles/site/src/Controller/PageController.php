...
class PageController extends SimpleController
{
    ...
    public function pageHealthCheckAdminPanel($request, $response, $args){
        $ms = $this->ci->alerts;
        /** @var UserFrosting\Sprinkle\Account\Model\User $currentUser */
        $currentUser = $this->ci->currentUser;
        /** @var UserFrosting\Sprinkle\Account\Authenticate\Authenticator $authenticator */
        ...
        // Return 200 success if user is already logged in
        if ($authenticator->check()) {
            return $this->ci->view->render($response, 'pages/healthCheck.html');
        }else{
            return $response->withRedirect($this->ci->router->pathFor('loginAdminPanel'), 302);
	    }
    }

    public function pageLogRecordsAdminPanel($request, $response, $args){
        $ms = $this->ci->alerts;
        /** @var UserFrosting\Sprinkle\Account\Model\User $currentUser */
        $currentUser = $this->ci->currentUser;
        /** @var UserFrosting\Sprinkle\Account\Authenticate\Authenticator $authenticator */
        ...
        // Return 200 success if user is already logged in
        if ($authenticator->check()) {
            return $this->ci->view->render($response, 'pages/logRecords.html');
        }else{
            return $response->withRedirect($this->ci->router->pathFor('loginAdminPanel'), 302);
	    }
    }
